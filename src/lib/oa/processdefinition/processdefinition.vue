...
<template lang='pug'>
  div
    keep-alive
      kalix-table(v-bind:isShowToolBar='isShowToolBar' bizKey='processDefinition' title='流程定义列表'
...
<script type='text/ecmascript-6'>
  ...
  import ProcessDefinitionView from '../comp/processDefinitionView'

  // Vue.component('ProcessDefinitionSearch', require('./processDefinitionSearch').default)

  export default {
    name: 'kalix-oa-processdefinition',
    activated() {
      ...
      customTableTool(row, btnId) {
        switch (btnId) {
          case 'processView': { // 待办功能，未实行
            this.$KalixEventBus.$emit('processDefinition' + '-' + this.$KalixEventConfig.ON_INIT_DIALOG_DATA, row)
