...
<script type="text/ecmascript-6">
  ...
  export default {
      ...
      var validateEndTime = (rule, value, callback) => {
        if (this.endTime === '' || this.endTime === null) {
          ...
          this.axios.get(MeetingroomApplyURL, {}).then(response => {
            ...
            let tempDate2 = ''
            let hasConfic = false
            if (this.meetingRoomApplyList && this.meetingRoomApplyList.length) {
              for (let i = 0; i < this.meetingRoomApplyList.length; i++) {
                tempDate = this.$M_FormatDate(new Date(this.meetingRoomApplyList[i].meetingDate), 'yyyy-MM-dd ')
                tempDate2 = this.$M_FormatDate(new Date(this.meetingDate), 'yyyy-MM-dd ')
                ...
                if (this.meetingRoomApplyList[i].status !== 0) {
                  if (tempDate2 === tempDate) {
                    ...
                    if (this.$M_FormatDate(new Date(this.endTime), 'hh:mm:ss ') <= this.meetingRoomApplyList[i].beginTime) {
                    } else if (this.$M_FormatDate(new Date(this.beginTime), 'hh:mm:ss ') >= this.meetingRoomApplyList[i].endTime) {
                    } else {
                      hasConfic = true
    ...
    methods: {
      ...
      getStartTime(val) {
        this.formModel.beginTime = this.$M_FormatDate(new Date(val), 'hh:mm:ss')
        this.formModel.beginTimeStr = this.$M_FormatDate(new Date(val), 'hh:mm:ss')
      },
      getEndTime(val) {
        this.formModel.endTime = this.$M_FormatDate(new Date(val), 'hh:mm:ss')
        this.formModel.endTimeStr = this.$M_FormatDate(new Date(val), 'hh:mm:ss')
      },
      getMeetingDate(val) {
        let meetingDate = this.$M_FormatDate(new Date(val), 'yyyy-MM-dd')
